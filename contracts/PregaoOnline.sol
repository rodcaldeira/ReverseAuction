...
// function to addNotice, create a new notice and pass by parameter the address of responsable for the notice: _buyer
  function addNotice(address _buyer) public {
    Notice n = new Notice(_buyer);
    notices.push(address(n));
  }

// return the array with all notices deployed by this factory
...
contract Notice {
  ...
  enum NoticeStatus {CONFIG, RUNNING, SUSPENDED, CANCELED, ENDED }
  ...
  uint i_start_eminent_bn;
  uint i_end_notice;
  uint public i_open_to_eminent;
  uint blocks_extensions;

  uint public i_suspension_init;
  ...
  modifier autoUpdateStatus() {
    if(block.number > i_end_notice) {
      noticeStatusHistory.push(noticeStatus);
      noticeStatus = NoticeStatus.ENDED;
      emit UpdateNotice(msg.sender, noticeStatus);
    }
    _;
  }
  ...
  */
  ...
  function endNotice() public notSuspended() onlyNoticeOwner() {
    require(block.number >= i_end_notice, "you cannot end");
    updateStatus(NoticeStatus.ENDED);
  }
  ...
  */
  ...
  function makeBid(address notice_item, uint offer) public autoUpdateStatus() notSuspended() notCanceled() onlyBidders() openToBid() {
  ...
  */
  ...
  function getInstanceStartEminent() public onlyNoticeOwner view returns (uint) {
    return i_start_eminent_bn;
  }

  function getInstanceEndNotice() public onlyNoticeOwner view returns (uint) {
    return i_end_notice;
  }

  function getInstanceBlockExtension() public onlyNoticeOwner view returns (uint) {
    return blocks_extensions;
  }
}
