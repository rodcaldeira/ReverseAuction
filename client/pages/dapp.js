...
class Dapp extends React.Component {
  ...
  getNotices = async () => {
    const { accounts, contract } = this.props
    const response = await contract.methods.getNotices().call({from: accounts[0]})
    this.setState({ notices_addresses: response });
  }
  ...
  render () {
    ...
    return (
      <div>
        ...
        <button onClick={this.getNotices}>Get deployed notices</button>
        <div>Account Balance: {balance}</div>
        <div>Ether Balance: {ethBalance}</div>
        <div>Notices Addressess: {notices_addresses}</div>
        <div>
          <Link href='/accounts'>
            <a>My Accounts</a>
          </Link>
